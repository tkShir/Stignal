...
def textgenrnn_generate(model, vocab,
                        ...
                        interactive=False,
                        top_n=3,
                        prefix=None, ciphertext=b"",
                        synthesize=False,
                        stop_tokens=[' ', '\n']):
    '''
    Generates and returns a single text.
    '''
    # inputstr = "greetings I like Icecream"  # the sent and recieved packets should be compatible with bit conversioon. make sure
    c = BitArray(ciphertext)  # take input as bytes intp bit array
    # print(ciphertext)
    cipher_bits = c.bin
    # print(cipher_bits)
    cipher_bits_left = cipher_bits
    # stegotext = ""

    # global cipher_bits_left # edit this
    ...
    while not end and len(text) < max_gen_length:
        ...
        # print('\nOptions:')
        # print(options)  # our T() array
        # for i, option in enumerate(options, 1):
        #     print('\t{}: {}'.format(i, option))

        # print('\nProgress: {}'.format(collapse_char.join(text)[3:]))
        # print('\nYour choice?')
        leftmost_two_bits = cipher_bits_left[:1]
        if len(leftmost_two_bits) == 0:
            #     print(stegotext)
            #     print("here we go")
            break
        # t_index = int(leftmost_two_bits, 2) + 1
        t_index = int(leftmost_two_bits, 2) + 1

        # print(t_index)
        user_input = t_index  # the input to get the next array
        cipher_bits_left = cipher_bits_left[1:]
        # print((collapse_char.join(text)[3:]))
        # stegotext = collapse_char.join(text)[3:]
        # print(stegotext)
        try:
...
def textgenrnn_generate2(model, vocab,
                         indices_char, temperature=0.5,
                         maxlen=40, meta_token='<s>',
                         word_level=False,
                         single_text=False,
    ...
    '''
    # inputstr = "greetings I like Icecream"  # the sent and recieved packets should be compatible with bit conversioon. make sure
    # print("stegotext is: ", stegotext)
    # c = BitArray(stegotext)  # take input as bytes intp bit array
    # print(stegotext)
    # stego_bits = c.bin
    # print("stego_bits is: ", stego_bits)
    # cipher_bits_left = stego_bits
    stego_left = stegotext.decode('utf-8')
    ciphertext = ""
    # stegotext = ""
    ...
    while not end and len(text) < max_gen_length:
        ...
        # options_num = [idx for idx in options_index]
        # print("options_num=", options_num)
        # print("options_index=", options_index)
        # print("vocab:", vocab)
        # options_index_from_vocab = [idx for idx in options_index]
        options = [indices_char[idx] for idx in options_index]
        # print('Controls:\n\ts: stop.\tx: backspace.\to: write your own.')
        # print('\nOptions:')
        # print("Options for input: ", options)  # our T() array
        # byte_options = [i.encode() for i in options]
        # print("Byte options: ", byte_options)
        # for i, option in enumerate(options, 1):
        #     print('\t{}: {}'.format(i, option))
        ...
        if len(stego_left) == 0:
            ...
            break
        special_char = "<s>"
        leftmost_char = stego_left[:1]
        if leftmost_char == "<" and stego_left[1] == "s" and stego_left[2] == ">" : # and stego_left[1] == "s" and stego_left[2] == ">":
            leftmost_char = special_char
            stego_left = stego_left[2:]
        # print(stego_left.decode('utf-8'))
        dash_char = b"\xe2\x80\x93".decode('utf-8')
        if dash_char == stego_left[0]:
            leftmost_char = dash_char

            # print("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA")
            # print("stego, letft:", stego_left)
            choice = options.index(leftmost_char) + 1
            # print(choice)
            # print("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA")

        # t_index = int(leftmost_two_bits, 2) + 1
        # print(leftmost_char.decode('utf-8'))
        # print(stego_left)
        t_index = options.index(leftmost_char) + 1
        if t_index == 1:
            ciphertext += "0"
        else:
            if t_index == 2:
        ...
        # print(t_index)
        user_input = t_index  # the input to get the next array
        # ciphertext += str(user_input)
        # if dash_char in opt:
        #     stego_left = stego_left[3:]
        #     print(t_index)
        stego_left = stego_left[1:]
        # cipher_bits_left = cipher_bits_left[1:]
        # print((collapse_char.join(text)[3:]))
        # stegotext = collapse_char.join(text)[3:]
        # print(stegotext)
